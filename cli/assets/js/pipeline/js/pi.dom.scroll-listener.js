...
π.dom.scrollListener = {
  ...
  __elementInViewport = function(el){
    ...
    // Returns a bitmask for visibility of all edges of element
    // If bitmask is zero, element is outside of viewport

    return Number(rect.top  >= 0)
        & (Number(rect.left >= 0) << 1)
        & (Number(rect.bottom <= window.innerHeight) << 2)
        & (Number(rect.right <= window.innerWidth) << 3);
    },
