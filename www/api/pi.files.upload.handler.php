...
class UploadHandler
{
    ...
    private $redis = null;
    private $servicekey = "views:upload:batch";
    ...
    function __construct($options = null, $initialize = true) {
        $this->options = array(
            ...
            //'upload_dir' => '/var/www/upload/views/batch',
	...
	private function log($msg, $data=null){
		$this->redis->lPush($this->servicekey, json_encode($msg));
  }

  private function closeRedis(){
		try{	
			$this->log("EXITING.", EVENT_CLOSE);
			$this->redis->close();
			return true;
		}
	...
	private function connectToRedis(){
		...
		try{	
			...
			switch($redis->getOption(Redis::OPT_SERIALIZER)){
				case SERIALIZER_NONE: $msg="SERIALIZER_NONE"; break;
				case SERIALIZER_IGBINARY: $msg="SERIALIZER_IGBINARY"; break;
				case SERIALIZER_PHP: $msg="SERIALIZER_PHP"; break;
				case false: $msg="FUCK_ALL. There arose an unfortunate situation when we tried to get something as trivial as the option setting for OPT_SERIALIZER. This does not bode well for the rest of our script. Nonetheless, we will do our best.Please excuse any inconvenience caused. - The Manager."; sleep(15); break;
				default: $msg="SERIALIZER_UNKNOWN";
			} 
//			$redis->select(15);
			$this->log("Connected to Redis using $msg.");
			return $redis;
		}
		catch(RedisException $e){
			//$this->log("Redis exception ({$config['redis']['channel']}):". $e->getMessage());
			return false;
		}
